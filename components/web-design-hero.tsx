...
const webDesignImages = [
  ...
  '/web_dev/57042bdd7ab66e1a.png',
  '/web_dev/96be545d935.png',
]
...
export default function WebDesignHero() {
  ...
  useEffect(() => {
    if (webDesignImages.length <= 1) return
    const timer = setInterval(() => {
      setCurrent((prev) => (prev + 1) % webDesignImages.length)
    }, 5000)
    return () => clearInterval(timer)
  }, [])

  const next = () => setCurrent((prev) => (prev + 1) % webDesignImages.length)
  const prev = () =>
    setCurrent((prev) => (prev - 1 + webDesignImages.length) % webDesignImages.length)
    ...
    <section className="relative h-screen overflow-hidden pt-24 bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      ...
      <div className="absolute inset-0">
        <AnimatePresence mode="wait">
          ...
          >
            <Image
              src={webDesignImages[current]}
              alt={`Web design showcase ${current + 1}`}
              fill
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-white text-center px-6 max-w-4xl mx-auto">
        ...
        >
          Conversion-Focused Websites for Product Brands
        </motion.h1>

        <motion.p
          className="text-lg sm:text-xl md:text-2xl font-light mb-12 opacity-95 max-w-3xl leading-relaxed drop-shadow-lg"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.8, delay: 0.3 }}
        >
          Clean design, fast performance, and a structure built to sell.
        </motion.p>
